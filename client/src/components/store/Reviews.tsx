...
import ChooseReviewRating, {
  ChooseReviewRatingHandle,
} from "./ChooseReviewRating";
...
export default function Reviews(props: Props) {
  ...
  useEffect(() => {
    var sum = 0;
    if (
      props.product?.reviews != undefined &&
  ...
  async function onAddReview() {
    if (
      !titleInput.current?.isValid ||
      !commentInput.current?.isValid ||
      !nameInput.current?.isValid
    ) {
    ...
    try {
      ...
      var result = await authClient.post(
        `${baseProductsURL()}api/products/add-review`,
        data,
      );
  ...
  async function onRemoveReview(review: ProductReview) {
    try {
      ...
      await authClient.post(
        `${baseProductsURL()}api/products/remove-review`,
        data,
      );
  ...
  return (
    <section className="grid">
      <h1 className="justify-self-center pb-4 pt-10 text-2xl font-bold">
        {t("reviews")}
      </h1>
        ...
        <div className="flex flex-col items-center">
          <div className="flex w-full text-black dark:text-white">
            <div className="flex w-full flex-col">
              ...
              <span>
                {props.product?.reviews.length} {t("reviews")}
              </span>
              <div className="flex items-center gap-1">
                ...
                <ProgressBar
                  className="grow"
                  percent={
                    (props.product?.reviews.filter((p) => p.rating == 5)
                      .length! /
                ...
                />
                <span>
                  {props.product?.reviews.filter((p) => p.rating == 5).length}
                </span>
              </div>
              <div className="flex items-center gap-1">
                <span>
                ...
                <ProgressBar
                  className="grow"
                  percent={
                    (props.product?.reviews.filter((p) => p.rating == 4)
                      .length! /
                      props.product?.reviews.length!) *
                    100
                  }
                />
                <span>
                  {props.product?.reviews.filter((p) => p.rating == 4).length}
                </span>
              </div>
              <div className="flex items-center gap-1">
                <span>
                ...
                <ProgressBar
                  className="grow"
                  percent={
                    (props.product?.reviews.filter((p) => p.rating == 3)
                      .length! /
                      props.product?.reviews.length!) *
                    100
                  }
                />
                <span>
                  {props.product?.reviews.filter((p) => p.rating == 3).length}
                </span>
              </div>
              <div className="flex items-center gap-1">
                <span>
                ...
                <ProgressBar
                  className="grow"
                  percent={
                    (props.product?.reviews.filter((p) => p.rating == 2)
                      .length! /
                      props.product?.reviews.length!) *
                    100
                  }
                />
                <span>
                  {props.product?.reviews.filter((p) => p.rating == 2).length}
                </span>
              </div>
              <div className="flex items-center gap-1">
                <span>
                ...
                <ProgressBar
                  className="grow"
                  percent={
                    (props.product?.reviews.filter((p) => p.rating == 1)
                      .length! /
                      props.product?.reviews.length!) *
                    100
                  }
                />
                <span>
                  {props.product?.reviews.filter((p) => p.rating == 1).length}
                </span>
              </div>
            </div>
            <div></div>
          ...
          {user.accessToken == null || user.accessToken == "" ? (
            <div>{t("reviewLoginRequired")}</div>
          ) : (
            <Modal submit={onAddReview}>
              <Modal.Button>
                <button
                  className="w-40 rounded-lg bg-orange-600 px-5
                                 py-2 text-white hover:bg-orange-700"
                >
                  {t("addReview")}
                </button>
              </Modal.Button>
              ...
              {props.product.reviews.map((item, index) => {
                return (
                  <div key={index}>
                    <div className="flex">
                      <div className="grid w-full grid-cols-2 md:grid-cols-3">
                        <div className="flex flex-col items-center justify-center">
                          ...
                          <h1>{item.name}</h1>
                          <h1>
                            {moment(new Date(item.addedDate)).format(
                              "DD-MM-YYYY HH:mm:ss",
                            )}
                          </h1>
                        </div>
                    ...
                    <div className="m-5 border-b border-gray-300" />
                  </div>
                );
              })}
            </div>
