...
export default function OrderPage() {
    ...
    return (
        <div className="flex flex-col gap-2 mt-2 mx-2 md:mx-0">
            <h1 className="my-2 text-2xl font-bold">{t('orderDetails')}</h1>
            {isLoading ?
                ...
                <section className="grid grid-cols-3 p-2 rounded-lg bg-white dark:bg-gray-700 shadow-lg">
                    <div className="col-span-2">
                        ...
                        <div className="grid grid-cols-2">
                            ...
                            <span>{order?.orderSum} {t("lv")}</span>
                        </div>
                        <div className="grid grid-cols-2">
                            <span>{t("status")}: </span>
                            <span>{t(OrderStatusEnum[order?.status ?? 0])}</span>
                        </div>
                    </div>
                </section>
            }
                        ...
                        return (
                            <div key={index} className="flex gap-2">
                                ...
                                <div className="flex flex-col">
                                    ...
                                    <span>{(Number.parseFloat(orderItem.sum) / Number.parseInt(orderItem.count)).toFixed(2)} {t('lv')}.</span>
                                    <span>{orderItem.count} {t('cnt')}.</span>
                                </div>
                            </div>
                        )
                    })}
